...
var _prng = require('../.sim-dist/prng');
 ...
 class RandomTeams {
	 ...
	 */
	statCalc(baseStats, ivs, evs, level, nature) {
		...
		const realStats = {hp: 0, atk: 0, def: 0, spa: 0, spd: 0, spe: 0};
		const evMod = this.gen > 3 ? (ev) => Math.floor(ev / 4) : (ev) => Math.floor(Math.ceil(Math.sqrt(ev)) / 4);
	...
	levelBalance(baseStats, ivs, evs, nature,
		averageIV = 0, averageEV = 0, weakling) {
		let realStats = this.statCalc(baseStats, ivs, evs, 100, nature);
